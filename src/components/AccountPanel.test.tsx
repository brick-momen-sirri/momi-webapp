...
//
// Most of this file's behavior lives behind the settings modal, which is why it
// only rendered a summary card before these tests existed.
...
// The account-information grid renders each entry as a label paragraph followed by
// a value paragraph. Reading the value via the label avoids matching the same word
// somewhere else in the modal -- "Admin" is also a role <option>, for instance.
function infoValue(dialog: HTMLElement, label: string) {
  return within(dialog).getByText(label, { selector: "p" }).nextElementSibling?.textContent ?? "";
}
...
describe("settings modal", () => {
  it("opens on the settings button and closes again on Cancel", async () => {
    ...
    await client.click(screen.getByRole("button", { name: /^cancel$/i }));
    expect(screen.queryByRole("dialog")).toBeNull();
  });
  ...
  it("reports the account's role and status", async () => {
...
describe("password form", () => {
  it("passes current, new, and confirmation values in that order", async () => {
    ...
    await client.type(screen.getByPlaceholderText("Current password"), "old-value");
    await client.type(screen.getByPlaceholderText("New password"), "new-value");
    await client.type(screen.getByPlaceholderText("Confirm new password"), "new-value");
    await client.click(screen.getByRole("button", { name: /change password/i }));

    expect(onChangePassword).toHaveBeenCalledWith("old-value", "new-value", "new-value");
  });
  ...
  it("keeps what was typed when the change is rejected", async () => {
    ...
    await client.type(screen.getByPlaceholderText("Current password"), "wrong-value");
    await client.click(screen.getByRole("button", { name: /change password/i }));

    expect(await screen.findByText("Current password is incorrect.")).toBeInTheDocument();
    // Not cleared: retyping everything after a typo is worse than leaving it.
    expect(screen.getByPlaceholderText("Current password")).toHaveValue("wrong-value");
  });
});
...
describe("admin user management", () => {
  it("is hidden from a non-admin account", async () => {
    ...
    expect(within(dialog).queryByPlaceholderText(/search users/i)).toBeNull();
  ...
  it("is available to an admin account", async () => {
    ...
    expect(within(dialog).getByPlaceholderText(/search users/i)).toBeInTheDocument();
  ...
  it("narrows the user list by search text", async () => {
    ...
    await client.type(within(dialog).getByPlaceholderText(/search users/i), "nour");
  ...
  it("matches the search against email as well as name", async () => {
    ...
    await client.type(within(dialog).getByPlaceholderText(/search users/i), "nour.ahmad@");
  ...
  it("hands a new user's details to the create callback", async () => {
    ...
    await client.type(within(dialog).getByPlaceholderText(/name\.surname@brickvisual\.com/i), "new.hire@brickvisual.com");
    await client.type(within(dialog).getByPlaceholderText("Temporary password"), "temp-value-1");
