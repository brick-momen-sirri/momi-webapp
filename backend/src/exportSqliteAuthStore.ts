...
export async function exportSqliteAuthStoreToJson(options: ExportOptions) {
  ...
  try {
    const users = db
      .prepare("SELECT data FROM auth_users ORDER BY updated_at DESC, id ASC")
      .all()
    ...
    `,
      ...
      .all()
      .map((row) => toSession(row as SessionRow));
    const projects = options.projectsPath
      ? db
          .prepare("SELECT data FROM app_projects ORDER BY created_at DESC, id ASC")
          .all()
    ...
    if (!users.length && !options.allowEmptyUsers) {
      throw new Error(
        "Refusing to export an app-state database with no users. Pass allowEmptyUsers only for deliberate recovery.",
      );
    }
    if (projects && !projects.length && !options.allowEmptyProjects) {
      throw new Error(
        "Refusing to export an app-state database with no projects. Pass allowEmptyProjects only for deliberate recovery.",
      );
    }
